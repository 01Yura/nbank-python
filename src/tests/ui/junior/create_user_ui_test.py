...
def handle_create_user_dialog(page: Page, message: str) -> None:
...
class CreateUserUiTest(BaseUiTest):
    def test_admin_can_create_user_with_valid_credentials(self, page: Page, api_manager: ApiManager):
        ...
        handle_create_user_dialog(page, "User created successfully!")
    ...
    def test_admin_cannot_create_user_with_invalid_credentials(self, page: Page, api_manager: ApiManager):
        ...
        # Проверяем, что alert появился и текст сообщения соответствует ожидаемому
        handle_create_user_dialog(page, "Failed to create user")
